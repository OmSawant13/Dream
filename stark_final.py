...
F.R.I.D.A.Y. with a REAL brain:
  • Multi-model: Gemini 2.5 Flash (cloud) + Ollama (local, free, unlimited)
...
OLLAMA_MODEL = "llama3.2"           # Change to "qwen2.5:7b" or "mistral" for better tool use
...
def run():
    ...
    use_gemini = provider in ("gemini", "auto") and api_key
    gemini_chat = None
    ollama_tools = None
    ollama_messages = []
    ...
    if use_gemini:
        try:
            ...
            gemini_chat = model.start_chat(enable_automatic_function_calling=True)
            brain_name = f"Gemini 2.5 Flash"
        except Exception as e:
            print(f"⚠ Gemini init failed ({e}). Falling back to Ollama.")
            use_gemini = False

    # ── Initialize Ollama as primary or fallback ──
    if not use_gemini or provider == "auto":
        ollama_tools = _build_ollama_tools()
        ollama_messages = [{"role": "system", "content": SYSTEM_PROMPT}]
        if not use_gemini:
            brain_name = f"Ollama {OLLAMA_MODEL} (LOCAL/FREE)"

    print("\n" + "═"*55)
    print("  F.R.I.D.A.Y. NEXUS PROTOCOL — MARK L")
    print(f"  Brain: {brain_name} | Tools: {len(ALL_TOOLS)} Real Modules")
    print(f"  Mode: {provider.upper()} | Memory: ACTIVE")
    print("═"*55 + "\n")
    ...
    while True:
        try:
            ...
            if cmd.lower().startswith("/model"):
                parts = cmd.split()
                if len(parts) >= 2 and parts[1].lower() == "ollama":
                    use_gemini = False
                    if not ollama_tools:
                        ollama_tools = _build_ollama_tools()
                        ollama_messages = [{"role": "system", "content": SYSTEM_PROMPT}]
                    print("F.R.I.D.A.Y.: Switched to Ollama (local/free). No credits needed, boss.")
                elif len(parts) >= 2 and parts[1].lower() == "gemini":
                    if api_key:
                        use_gemini = True
                        if not gemini_chat:
                            m = genai.GenerativeModel(GEMINI_MODEL, system_instruction=SYSTEM_PROMPT, tools=ALL_TOOLS)
                            gemini_chat = m.start_chat(enable_automatic_function_calling=True)
                        print("F.R.I.D.A.Y.: Switched to Gemini 2.5 Flash (cloud).")
                    else:
                        print("F.R.I.D.A.Y.: No API key configured for Gemini, boss.")
                else:
                    print("F.R.I.D.A.Y.: Usage: /model ollama  or  /model gemini")
                continue
            ...
            if use_gemini and gemini_chat:
                try:
                    ...
                    err = str(e).lower()
                    if any(k in err for k in ["quota", "429", "resource", "exhausted", "limit", "billing"]):
                        print("\n⚡ Gemini credits exhausted. Auto-switching to Ollama (free)...")
                        use_gemini = False
                        if not ollama_tools:
                            ollama_tools = _build_ollama_tools()
                            ollama_messages = [{"role": "system", "content": SYSTEM_PROMPT}]
                        ...
                        print(f"\n⚠ Gemini error: {e}")
                        print("  Routing through Ollama instead...")
            ...
            if reply is None:
                if not ollama_tools:
                    ollama_tools = _build_ollama_tools()
                    ollama_messages = [{"role": "system", "content": SYSTEM_PROMPT}]
                ollama_messages.append({"role": "user", "content": cmd})
                reply = _ollama_chat(ollama_messages, ollama_tools)
                ollama_messages.append({"role": "assistant", "content": reply or ""})
